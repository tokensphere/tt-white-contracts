...
contract IssuerFrontendFacet is AIssuerFacet {
   ...
   */
  function paginateDetailedFasts(
    uint256 cursor,
    uint256 perPage
  ) external view returns (FastFrontendFacet.Details[] memory, uint256) {
    (address[] memory addresses, uint256 nextCursor) = LibPaginate.addresses(
      LibIssuer.data().fastSet.values,
      cursor,
      perPage
    );
   ...
   * @return A `uint256` index to the next page.
   */
  function paginateDetailedFastsInGroup(
    string calldata group,
    uint256 cursor,
    uint256 perPage
  ) external view returns (FastFrontendFacet.Details[] memory, uint256) {
    (address[] memory addresses, uint256 nextCursor) = LibPaginate.addresses(
      LibIssuer.data().fastGroups[group].values,
      cursor,
      perPage
    );
    FastFrontendFacet.Details[] memory fastDetails = new FastFrontendFacet.Details[](addresses.length);
