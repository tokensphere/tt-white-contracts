...
- Emits a `Minted(amount, ref)` event._
...
- Calls `FastFrontendFacet.emitDetailsChanged`_
...
- Emits a `Transfer(from, to, amount)` event._
...
- Emits an `Approval(from, spender, amount)`._
...
_Business logic:
 ...
 - Otherwise,
   - Require that the candidate is a member of the Token contract._
